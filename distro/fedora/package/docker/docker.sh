...
remove_old_versions() {
    ...
    if sudo dnf remove -y "${packages[@]}" 2>&1 | sudo tee -a "$LOG_FILE"; then
...
setup_repository() {
    ...
    if sudo dnf config-manager addrepo --from-repofile https://download.docker.com/linux/fedora/docker-ce.repo 2>&1 | sudo tee -a "$LOG_FILE"; then
...
install_docker_packages() {
    ...
    if sudo dnf install docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin -y 2>&1 | sudo tee -a "$LOG_FILE"; then
...
configure_and_start_service() {
    ...
    if sudo systemctl enable --now docker 2>&1 | sudo tee -a "$LOG_FILE"; then
        ...
        sudo alternatives --set iptables /usr/bin/iptables-nft 2>&1 | sudo tee -a "$LOG_FILE" || true
        if sudo systemctl restart docker 2>&1 | sudo tee -a "$LOG_FILE"; then
...
configure_user_group() {
    ...
    if grep -q "^docker:" /etc/group; then
        ...
        sudo groupadd docker 2>&1 | sudo tee -a "$LOG_FILE" || true
    fi

    if sudo usermod -aG docker "$TARGET_USER" 2>&1 | sudo tee -a "$LOG_FILE"; then
...
verify_installation() {
    ...
    if sudo docker run --rm hello-world 2>&1 | sudo tee -a "$LOG_FILE"; then
